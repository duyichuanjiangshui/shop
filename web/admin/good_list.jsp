...
<div class="container-fluid">
	...
	<div  class="text-right"><a class="btn btn-warning" href="goodAdd" style=" position: relative;  top: 55px;">添加商品</a></div>
	
	<br>
	
	<form class="form-inline" action="goodSearch" id="serchform" method="post">
	...
	<ul role="tablist" class="nav nav-tabs">
        ...
        <li <c:if test='${status==3}'>class="active"</c:if> role="presentation"><a href="goodList?status=3">性价比推荐</a></li>
    </ul>
...
<%-- 				<c:if test="${good.topScroll}"><a class="btn btn-info topDelete" href="javascript:;" type="1" goodid="${good.id}" text="加入条幅">移出条幅</a></c:if>
				...
				<c:if test="${good.topSmall}"><a class="btn btn-info topDelete" href="javascript:;" type="3" goodid="${good.id}" text="加入新品">移出性价比</a></c:if>
				<c:if test="${!good.topSmall}"><a class="btn btn-primary topSave" href="javascript:;" type="3" goodid="${good.id}" text="移出新品">加入性价比</a></c:if>
				</p>
