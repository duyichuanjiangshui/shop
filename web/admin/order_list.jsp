...
<div class="container-fluid">
	...
	<form class="form-inline" action="orderSearch" id="serchform" method="post">
		<div class="form-group">
			<input type="text" class="form-control" name="id" placeholder="订单号" onkeyup="this.value=this.value.replace(/[^\d]+/g,'')">
		</div>
         ...
         <tr>
         	...
         	<td>
	         	<c:forEach var ="item" items="${order.itemList}">
		         	<p><a href="../index/detail?goodid=${item.good.id}" target="_blank">${item.good.name}</a>(${item.price}) x ${item.amount}</p>
	         	</c:forEach>
         	</td>
         	<td>
         		<p>${order.name}</p>
